      ...
      res.on("end", () => {
        ...
        if (res.statusCode === 200) {
          try {
            ...
            console.log(
              `   ✅ Success: ${response.user.name} (${response.user.role})`
            );
          } catch (e) {
            console.log(
              `   ✅ Success but response parsing failed: ${body.substring(
                0,
                100
              )}...`
            );
          ...
          try {
            ...
            console.log(
              `   ❌ Error: ${res.statusCode} - ${body.substring(0, 100)}...`
            );
          }
        }
        console.log("");
...
async function runTests() {
  ...
  console.log(
    "If you see ❌ Error, there may be an issue with the database or environment variables."
  );
}
